...
"""
    ...
    # 제미니 API 실제 가동부  
    try:
        # 1. 최신 Gemini 3.5 Flash 또는 3.1 Pro 모델로 명칭 변경 (구형 1.5에서 업그레이드)
        # 상황극과 복잡한 에이전트 규칙 처리에 가장 최적화된 최신 엔진입니다.
        model = genai.GenerativeModel(
            model_name="gemini-3.5-flash",  # 혹은 "gemini-3.1-pro" 사용 가능
            system_instruction=system_prompt
        )

        # 2. 기존 대화 히스토리(chat_history)를 API 포맷에 맞게 빌드
        history_param = []
        for chat in st.session_state.chat_history:
            history_param.append({"role": "user", "parts": [chat["user"]]})
            history_param.append({"role": "model", "parts": [chat["reply"]]})

        # 3. 과거 기억 세션을 유지한 채 실시간 대답 자동 생성 요청
        chat_session = model.start_chat(history=history_param)
        response = chat_session.send_message(user_input)
        
        # 4. 생성 완료된 최종 대답 텍스트 추출
        gemini_reply = response.text

    except Exception as e:
        # 엔진 작동 과정에서 다른 충돌이 나면 멈추지 않고 화면에 기록
        gemini_reply = f"⚠️ [제미니 최신 모델 구동 실패]: {str(e)}"
